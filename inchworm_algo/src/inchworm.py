...
<<<<<<< HEAD
            ...
            for i in range(height ):
=======
            # rospy.loginfo(shingle_index_order)
        else:
            for i in range(height):
                # rospy.loginfo(i%2)
>>>>>>> 355448d00e84c789b74f4edb291c5f91fadbace1
...
>>>>>>> 355448d00e84c789b74f4edb291c5f91fadbace1
    ...
    def place_shingle(self, ee, shingle, roof):
        if (ee == 1):
            ...
            self.roof[self.bottom_foot_position[0] +
                      self.bottom_foot_position[1] * self.roof_width] = 1
            ...
            self.roof[self.top_foot_position[0] +
                      self.top_foot_position[1] * self.roof_width] = 1
    ...
    def pickup_shingle(self, ee, shingle, roof):
        ...
        if (ee == 1):
            self.roof[self.bottom_foot_position[0] +
                      self.bottom_foot_position[1] * self.roof_width] = 0
            self.bottom_foot_shingle_stat = EEShingleStatus.ATTACHED
        else:
            self.roof[self.top_foot_position[0] +
                      self.top_foot_position[1] * self.roof_width] = 0
    ...
    def ee_to_move_to_free_space(self, neighbors, ee_to_move):
        move_targets = []
        for n in neighbors:
            if self.roof[n[1] * self.roof_width + n[0]] != 2:
...
<<<<<<< HEAD
        for coord in self.shingle_order:
            val = self.roof[coord[1] * self.roof_width + coord[0]]
            if val != 2:
                x_coord = coord[0]
                y_coord = coord[1]
                rospy.loginfo(f"inchworm {self.id} setting tile {coord} as target")
                ...
                for row in shingles:
                    for element in row:
                        if element is not None:
                            self.roof.append(element.shingle_status.value)
                        else:
                            self.roof.append(0)
...
<<<<<<< HEAD
                # rospy.loginfo(f"inchworm {self.id} ee1 next to placed shingles is: {self.next_to_placed_shingle(self.ee1_position, shingles)}")
                # rospy.loginfo(f"inchworm {self.id} ee2 next to placed shingles is: {self.next_to_placed_shingle(self.ee2_position, shingles)}")
=======
...
>>>>>>> 355448d00e84c789b74f4edb291c5f91fadbace1
                    ...
                    if Inchworm.dist(self.avg_pos, self.target) > Inchworm.dist([placed_shingle.x_coord, placed_shingle.y_coord], self.target):
                        ...
                        if Inchworm.dist([placed_shingle.x_coord, placed_shingle.y_coord], self.target) == 0.0:
                            ...
                            rospy.logwarn(
                                f"inchworm {self.id} -top_foot- {self.ee_shingle_neighbors}")
                            rospy.logwarn(
                                f"inchworm {self.id} -bottom_foot- {self.get_ee_neighbors(self.bottom_foot_position)}")

<<<<<<< HEAD
                            self.ee_shingle_neighbors = self.ee_to_move_to_shingle(self.get_ee_neighbors(self.ee2_position), 'ee1')
...
=======
                            ...
                            rospy.logwarn(
                                f"inchworm {self.id} moving bottom_foot using position")
>>>>>>> 355448d00e84c789b74f4edb291c5f91fadbace1
...
=======
                            ...
                            rospy.logwarn(
                                f"inchworm {self.id} -top_foot- {self.get_ee_neighbors(self.bottom_foot_position)}")
                            rospy.logwarn(
                                f"inchworm {self.id} -bottom_foot- {self.get_ee_neighbors(self.top_foot_position)}")
                            rospy.logwarn(
                                f"inchworm {self.id} moving top_foot using position")
>>>>>>> 355448d00e84c789b74f4edb291c5f91fadbace1
...
<<<<<<< HEAD
                            ...
                            if (len(self.ee_shingle_neighbors) > 0 and self.roof[self.ee_shingle_neighbors[0]["pos"][1] * self.roof_width  + self.ee_shingle_neighbors[0]["pos"][0]] == 2 and
...
<<<<<<< HEAD
                        ...
                        if Inchworm.dist([placed_shingle.x_coord, placed_shingle.y_coord], self.target) == 0:
                            ...
                            ee2_shingle_neighbors = self.ee_to_move_to_free_space(self.get_ee_neighbors(self.ee2_position), 'ee1')
                            ee1_shingle_neighbors = self.ee_to_move_to_free_space(self.get_ee_neighbors(self.ee1_position), 'ee2')
                            ee2_shingle_neighbors.sort(key = lambda x: Inchworm.dist(x["pos"], self.target))
                            ee1_shingle_neighbors.sort(key = lambda x: Inchworm.dist(x["pos"], self.target))
                            
                            # below is useful for making sure that the hex lib is working correctly
                            # rospy.loginfo(f"placed shingle location {placed_shingle_location}")
                            # rospy.loginfo(f"inchworm {self.id} ee1 dist to placed shingle :{Inchworm.dist(placed_shingle_location, self.ee1_position)}")
                            # rospy.loginfo(f"ee1 shingle neighbors: {ee1_shingle_neighbors}")
                            # rospy.loginfo(f"inchworm {self.id} ee2 dist to placed shingle :{Inchworm.dist(placed_shingle_location, self.ee2_position)}")
                            # rospy.loginfo(f"ee2 shingle neighbors: {ee2_shingle_neighbors}")


                            ee2_neighbors = [value for elem in ee2_shingle_neighbors for value in elem.values()]
                            ee1_neighbors = [value for elem in ee1_shingle_neighbors for value in elem.values()]
                            
                            if placed_shingle_location in ee2_neighbors and placed_shingle_location not in ee1_neighbors: # figure out which ee needs to move to get closer to target
                                self.ee_shingle_neighbors = self.ee_to_move_to_free_space(self.get_ee_neighbors(self.ee2_position), 'ee1')
                                self.ee_shingle_neighbors.sort(key = lambda x: Inchworm.dist(x["pos"], self.target))
                                self.old_ee1 = self.ee1_position
                                if len(ee2_shingle_neighbors) > 0:
                                    inchworm_occ[ee2_shingle_neighbors[0]["pos"][1]][ee2_shingle_neighbors[0]["pos"][0]] = 1

                            elif placed_shingle_location in ee1_neighbors and placed_shingle_location not in ee2_neighbors: 
                                self.ee_shingle_neighbors = self.ee_to_move_to_free_space(self.get_ee_neighbors(self.ee1_position), 'ee2')
                                self.ee_shingle_neighbors.sort(key = lambda x: Inchworm.dist(x["pos"], self.target))
                                self.old_ee2 = self.ee2_position
                                if len(ee1_shingle_neighbors) > 0:
                                    inchworm_occ[ee1_shingle_neighbors[0]["pos"][1]][ee1_shingle_neighbors[0]["pos"][0]] = 1




                            elif len(ee1_shingle_neighbors) > 0 and len(ee2_shingle_neighbors) > 0 and Inchworm.dist(ee1_shingle_neighbors[0]["pos"], self.target) > Inchworm.dist(ee2_shingle_neighbors[0]["pos"], self.target): # figure out which ee needs to move to get closer to target
                                self.ee_shingle_neighbors = self.ee_to_move_to_free_space(self.get_ee_neighbors(self.ee2_position), 'ee1')
                                self.ee_shingle_neighbors.sort(key = lambda x: Inchworm.dist(x["pos"], self.target))
                                self.old_ee1 = self.ee1_position
                                inchworm_occ[ee2_shingle_neighbors[0]["pos"][1]][ee2_shingle_neighbors[0]["pos"][0]] = 1

                            elif len(ee1_shingle_neighbors) > 0 and len(ee2_shingle_neighbors) > 0 and Inchworm.dist(ee1_shingle_neighbors[0]["pos"], self.target) < Inchworm.dist(ee2_shingle_neighbors[0]["pos"], self.target): 
                                self.ee_shingle_neighbors = self.ee_to_move_to_free_space(self.get_ee_neighbors(self.ee1_position), 'ee2')
                                self.ee_shingle_neighbors.sort(key = lambda x: Inchworm.dist(x["pos"], self.target))
                                self.old_ee2 = self.ee2_position
                                inchworm_occ[ee1_shingle_neighbors[0]["pos"][1]][ee1_shingle_neighbors[0]["pos"][0]] = 1
                                ...
                                rospy.logwarn(f"inchworm {self.id} could not figure out which end effector to use to move the shingle at {placed_shingle_location}")

                            inchworm_occ[placed_shingle_location[1]][placed_shingle_location[0]] = 1
...
=======
                        ...
                        if len(bottom_foot_shingle_neighbors) > 0:
                            ...
                            rospy.loginfo(
                                f"closest bottom_foot neighbors {bottom_foot_shingle_neighbors[0]}")
                        if len(top_foot_shingle_neighbors) > 0:
                            rospy.loginfo(
                                f"top_foot shingle neighbors {top_foot_shingle_neighbors}")
                            rospy.loginfo(
                                f"closest top_foot neighbors {top_foot_shingle_neighbors[0]}")
                        ...
                        if placed_shingle_location in top_foot_neighbors and placed_shingle_location not in bottom_foot_neighbors:
                            self.ee_shingle_neighbors = self.ee_to_move_to_free_space(
                                self.get_ee_neighbors(self.top_foot_position), EE.BOTTOM_FOOT)
                            self.ee_shingle_neighbors.sort(
                                key=lambda x: Inchworm.dist(x["pos"], self.target))
                            self.old_bottom_foot = self.bottom_foot_position
                        ...
                        # for additional checks, make sure that both shingle lists have at least one entry
                            ...
                            if Inchworm.dist(bottom_foot_shingle_neighbors[0]["pos"], self.target) > Inchworm.dist(top_foot_shingle_neighbors[0]["pos"], self.target):
                                ...
                                self.old_bottom_foot = self.bottom_foot_position
                                rospy.loginfo(
                                    f"inchworm {self.id} moving bottom_foot based on shingle n")
                                inchworm_occ[top_foot_shingle_neighbors[0]["pos"]
                                                [1]][top_foot_shingle_neighbors[0]["pos"][0]] = 1
                            # if top_foot is farther away, move top_foot
                                ...
                                self.old_top_foot = self.top_foot_position
                                rospy.loginfo(
                                    f"inchworm {self.id} moving top_foot based on shingle n")
                                inchworm_occ[bottom_foot_shingle_neighbors[0]["pos"]
                                                [1]][bottom_foot_shingle_neighbors[0]["pos"][0]] = 1
...
<<<<<<< HEAD
            rospy.sleep(Inchworm.DELAY) # TODO: remove this later, just for testing
=======
            rospy.sleep(Inchworm.DELAY)
            rospy.loginfo(
                f"shingle depot at {shingle_depots[0].get_location()}")
...
=======
                    ...
                    if Inchworm.dist(self.bottom_foot_position, self.target) > Inchworm.dist(self.top_foot_position, self.target):
                        self.decide_on_movement(EE.BOTTOM_FOOT)
                        self.installing_status = 2

                    else:
                        self.decide_on_movement(EE.TOP_FOOT)
>>>>>>> 355448d00e84c789b74f4edb291c5f91fadbace1
                        self.installing_status = 2
...
=======
                            self.roof[self.install_shingle_target.y_coord * self.roof_width +
                                      self.install_shingle_target.x_coord] = ShingleStatus.INSTALLED.value
                            inchworm_occ[self.old_bottom_foot[1]][self.old_bottom_foot[0]] = 0
                            rospy.loginfo(
                                f"cleaning {[self.old_bottom_foot[1],self.old_bottom_foot[0]]}")
>>>>>>> 355448d00e84c789b74f4edb291c5f91fadbace1
...
=======
                            self.roof[self.install_shingle_target.y_coord * self.roof_width +
                                      self.install_shingle_target.x_coord] = ShingleStatus.INSTALLED.value
                            rospy.loginfo(
                                f"cleaning {[self.old_top_foot[1],self.old_top_foot[0]]}")
                            inchworm_occ[self.old_top_foot[1]][self.old_top_foot[0]] = 0
>>>>>>> 355448d00e84c789b74f4edb291c5f91fadbace1
                            rospy.sleep(Inchworm.DELAY)
                            rospy.loginfo(
...
<<<<<<< HEAD
            if self.move_shingle_step == 1:
                self.old_shingle_pos = [self.shingle_to_move.x_coord, self.shingle_to_move.y_coord]
                if self.ee_shingle_neighbors[self.ee_shingle_neighbor_index]["ee"] == "ee1":
                    if [self.shingle_to_move.x_coord, self.shingle_to_move.y_coord] != self.ee1_position:
                        # rospy.loginfo(f"inchworm {self.id} moving ee1 to {[self.shingle_to_move.x_coord, self.shingle_to_move.y_coord]}")
...
<<<<<<< HEAD
                    if [self.shingle_to_move.x_coord, self.shingle_to_move.y_coord] != self.ee2_position:
                        # rospy.loginfo(f"inchworm {self.id} moving ee2 to {[self.shingle_to_move.x_coord, self.shingle_to_move.y_coord]}")
...
<<<<<<< HEAD
=======
            # TODO: the behavoir does not work currently, we may or may not need this depending on how we build up the other behaviors
            rospy.loginfo(inchworm_occ)
>>>>>>> 355448d00e84c789b74f4edb291c5f91fadbace1
            if inchworm_occ[self.ee_shingle_neighbors[self.ee_shingle_neighbor_index]["pos"][1]][self.ee_shingle_neighbors[self.ee_shingle_neighbor_index]["pos"][0]] == 0:
                ...
                if self.ee_shingle_neighbor_index >= len(self.ee_shingle_neighbors):
                    rospy.logwarn(
                        f"inchworm {self.id} is trying to move and found no shingles that it could move it towards it's goal")
                    if self.ee_shingle_neighbors[0]["ee"] == EE.BOTTOM_FOOT:
                        self.move_bottom_foot(self.old_bottom_foot)
                        inchworm_occ[self.bottom_foot_position[1]
                                     ][self.bottom_foot_position[0]] = 1
                    else:
                        self.move_top_foot(self.old_top_foot)
                        inchworm_occ[self.top_foot_position[1]
                                     ][self.top_foot_position[0]] = 1
                    self.robot_state = RobotState.MAKE_DECISION
            rospy.sleep(Inchworm.DELAY)
...
=======
            ...
            # TODO: this should change and move into a seperate function
            # this should be some path planning that works the same as moving toward the target
            if Inchworm.dist(self.bottom_foot_position, self.target) > Inchworm.dist(self.top_foot_position, self.target):
                self.decide_on_movement(EE.BOTTOM_FOOT)
...
>>>>>>> 355448d00e84c789b74f4edb291c5f91fadbace1
    ...
    def decide_on_movement(self, ee_to_move):
        if ee_to_move == EE.BOTTOM_FOOT:
            self.ee_shingle_neighbors = self.ee_to_move_to(
                self.get_ee_neighbors(self.top_foot_position), EE.BOTTOM_FOOT)
            self.ee_shingle_neighbors.sort(
                key=lambda x: Inchworm.dist(x["pos"], self.target))
            self.old_bottom_foot = self.bottom_foot_position
